...
function getSalesByDay(data, yearlySales, from, to, id) {
  ...
  const daysWorkedExcludingFridays = daysWorkedExcludingFridaysFun(
    fromDate,
    toDate
  );
...
function daysWorkedExcludingFridaysFun(from, to) {
  let workingDaysCount = 0;
...
const getSalesByDayFunctional = (yearlySales, from, to, id) => {
  return getSalesByDay(data, yearlySales, from, to, id);
};

console.log(getSalesByDayFunctional(5220, "2023-1-1", "2023-2-28", 1));